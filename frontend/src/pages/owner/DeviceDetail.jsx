...
export default function DeviceDetail() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [deviceRes, telemetryRes, faultsRes, predictionsRes] = await Promise.all([
        deviceService.getOwnerDevice(deviceId),
        telemetryService.getTelemetry(deviceId, 'owner'),
        telemetryService.getFaults(deviceId, 'owner'),
  ...
  const handleResolveFault = async (faultId) => {
    try {
      const response = await telemetryService.resolveFault(deviceId, faultId, 'owner');
      if (response.success) {
        fetchData();
      }
    } catch (error) {
  ...
  const handleExportPdf = async () => {
    try {
      await telemetryService.exportFaultsPdf(deviceId, 'owner');
    } catch (error) {
